...
class SideNav extends Component{
    ...
    constructor(props){
        ...
        if(this.state.url.endsWith('cadastrar')) {
            this.getPageConfig(this.state.url.replace('cadastrar', ''))
        }else {
            this.getPageConfig(this.state.url)
        }
    }


    getPageConfig = async (url) => {
        const result = await api.get(`${consts.BASE_URL}/${consts.API_URL}${url}pagina-config`)
        const {name, login} = result.data.customUserDetails
        const fotourl = result.data.customUserDetails.fotourl || image
        this.setState({...this.state, name, login, fotourl, sideNav: result.data.sideNavBeans})
    }

    getSideNavItens = async (url) => {
        const result = await api.get(`${consts.BASE_URL}/${consts.API_URL}${url}sidenav`)
        this.setState({sideNav: result.data})
        this.getUserInf(url)
    }

    getUserInf = async (url) => {
        const result = await api.get(`${consts.BASE_URL}/${consts.API_URL}${url}usuario-logado`)
