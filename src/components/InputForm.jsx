...
class InputHidden extends React.Component{
    componentDidMount() {
        this.props.input.onChange(this.props.inputConfig.value)
    }

    render() {
        return (<input {...this.props.input} value={this.props.inputConfig.value} type='hidden'/>)
    }
...
class RadioCheck extends React.Component{
    componentDidMount(){
        const el = document.getElementsByName(`${this.props.input.name}`)
    ...
    renderCheckBox = () => {
        const {input, inputConfig, inputConfig:{radioCheckValuesBeans}} = this.props
        return (
        <CheckboxGroup  className='checkboxGroup'
                        checkboxDepth={2}
                        value={inputConfig.checked}
                        name={input.name}
                        onChange={(values) => this.handleCheckboxListChange(values, input)}>
                        <If test={inputConfig.mainLabel}>
                            <p>{inputConfig.mainLabel}</p>
                            <div className="divider divider-radiocheck"></div>
                        </If>
    ...
    renderRadio = () => {
        ...
        return (
            <RadioGroup className='radioGroup'
                        name={input.name}
                        onChange={(values) => this.handleCheckboxListChange(values, input)}
                        selectedValue={inputConfig.checked[0]}>
                        <If test={inputConfig.mainLabel}>
                            <p>{inputConfig.mainLabel}</p>
                            <div className="divider divider-radiocheck"></div>
                        </If>
...
export const Button = props =>
    <button className={`m-1 btn ${props.color}`} type={props.type} name="action" disabled={props.isLoading}>
        {props.label}
        <If test={props.icon}>
            <i className={`right ${props.icon.includes('fas') ? props.icon : 'material-icons'}`}>{props.icon.includes('fas') ? '' : props.icon}</i>
        </If>
    </button>
